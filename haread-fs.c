 ...
 *
 *
 * Initialized from https://github.com/libfuse/libfuse/wiki/Filesystems .
 * Specially the rofs one LICENCE : GPL v3
 */

#define FUSE_USE_VERSION 26

static const char *hareadFsVersion = "0.0.4-4";
...
// Key: file system path. Values:  1 => fs Ok, 0 => fs Blocks, -1 => Key does
// not exist
...
static char *translate_path(const char *path, const char *fs_root) {
  ...
  int has_trailing_slash =
      (root_len > 0 && fs_root[root_len - 1] == '/') ? 1 : 0;
...
int filldir(const char *path, void *buf, fuse_fill_dir_t filler,
            GHashTable *filesMap, const char *fs_root) {
  struct dirent *de;
  pthread_t thread_id = 0;
...
static int callback_read(const char *path, char *buf, size_t size, off_t offset,
 ...
 */
static int callback_setxattr(const char *path, const char *name,
 ...
 */
static int callback_getxattr(const char *path, const char *name, char *value,
                             size_t size) {
...
static void usage(const char *progname) {
  fprintf(stdout,
          "usage: %s comma,separated,list,of,underlying-fss-paths mountpoint "
          "[options]\n"
          "\n"
...
void *check_if_filesystem_blocks(void *fsno) {
  ...
  while (1) {
    ...
    if (thread_ids[current_thread] != 0) {
      ...
      if (pthread_timedjoin_np(thread_ids[current_thread], NULL, &timeout) !=
          0) {
...
int main(int argc, char *argv[]) {
  ...
  for (long t = 0; t < Fscount; t++) {
    int rc = pthread_create(&threads[t], NULL, check_if_filesystem_blocks,
                            (void *)t);
    if (rc) {
